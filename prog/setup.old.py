...
# Common Names
	...
	# a_id = Attached to ID
		# - used to track what object is attached to specific key.
...
def personnel(u_id, battle_ship, type, lvl, location): # This will hold the rules for all personnel assigned to the ship. If they are not assigned, they have three locations they can be in, the Media Room, the Mess Hall, and Dorms. Pilots have an extra called the Pilots lounge. (This will have a purpose later)
	object = "personnel"
	...
	if location == "None":
		...
		location = location
	personnel_id = db.player_items.insert({'name' : names()[1], 'object' : object, 'type' : type, 'title' : title[lvl], 'gender' : names()[0], 'indiscretion' : indiscretion, 'health' : health, 'u_id' : u_id, 'p_id' : battle_ship, 'skill' : skill,'timestamp' : now, 'created' : seconds, 'location' : location})


def compartments(u_id, battle_ship):
	...
	for a in range(15):
		a = a+1
		name = names[a]
		compartment_id = db.player_items.insert({'name' : name, 'object' : object, 'type' : type[name], 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'health' : health, 'primary_support' : primary_support[name], 'secondary_support' : secondary_support[name], 'compartments' : compartments[name] })
		db.player_items.update({'_id' : battle_ship},{"$set": {name : compartment_id}})
		min = min_support[name]
		for x in range(min):
			personnel(u_id, battle_ship, type[name], 1, name)
			
			
		

		
def reactor(u_id, battle_ship,lvl): # All details for all reactors. We need the User ID, the battle_ship ID, and the lvl of the item.
	name = {1: "DD-FG-40", 2 : "DD-FG-50", 3 : "DD-FG-65", 4 : "DD-FG-700", 5 : "DD-FG-850", 6 : "DD-FG-950"}
	object = "fusion_generator"
	...
	if battle_ship:
		reactor_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'effiency' : effiency[lvl], 'max_deuterium' : max_deuterium[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
		db.player_items.update({'_id' : battle_ship},{"$set": {'reactor' : reactor_id}})
		personnel(u_id, battle_ship, primary_support, 1, "Engineering Room")
	else:
		reactor_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'effiency' : effiency[lvl], 'max_deuterium' : max_deuterium[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : "", 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
		db.player_items.update({'_id' : cargo_bay_id},{"$set": {'cargo_bay_%d' %a : reactor_id}})
		

def engines(u_id, battle_ship,lvl): # All details for all engines. We need the User ID, the battle_ship ID, and the lvl of the item.
	...
	#Lets start some inserts :-)
	if battle_ship:
		engine_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'thrust' : thrust[lvl], 'type' : type[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
		db.player_items.update({'_id' : battle_ship},{"$set": {'reactor' : engine_id}})
		personnel(u_id, battle_ship, primary_support, 1, "Engine Room")
	else:
		engine_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'thrust' : thrust[lvl], 'type' : type[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : "", 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
		db.player_items.update({'_id' : cargo_bay_id},{"$set": {'cargo_bay_%d' %a : engine_id}})
		
		
def warp_drive(u_id, battle_ship,lvl): # All details for all Warp Drives. We need the User ID, the battle_ship ID, and the lvl of the item.
	...
	#That should be most of the variables 
	#Lets start some inserts :-)
	engine_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'kw_ly' : kw_ly[lvl], 'recharge' : recharge[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : "", 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})


def cannon(u_id, battle_ship,lvl): # All details for all reactors. We need the User ID, the battle_ship ID, and the lvl of the item.
	...
	#That should be most of the variables 
	#Lets start some inserts :-)
	cannon_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'rps' : rps[lvl], 'caliber' : caliber[lvl], 'accuracy' : accuracy[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})

	
def missile(u_id, battle_ship,lvl): # All details for all reactors. We need the User ID, the battle_ship ID, and the lvl of the item.
	...
	#That should be most of the variables 
	#Lets start some inserts :-)
	missile_id = db.player_items.insert({'name' : name[lvl], 'object' : object, 'guidance' : guidance[lvl], 'thrust' : thrust[lvl], 'accuracy' : accuracy[lvl], 'warhead' : warhead[lvl], 'range' : limit[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
	

def fighter(u_id, battle_ship, lvl, hanger_id): # At this moment I'm thinking each ship will be declared.
	name = "GF-37 Falcon"
	type = "Fighter"
	object = "Ship"
	...
	#That should be most of the variables 
	#Lets start some inserts :-)
	fighter_id = db.player_items.insert({'name' : name, 'object' : object, 'type' : type, 'range' : limit[lvl], 'life' : life[lvl], 'iron_cost' : iron_cost[lvl], 'carbon_cost' : carbon_cost[lvl], 'build_time' : build_time[lvl], 'lvl' : lvl, 'health' : health, 'u_id' : u_id, 'p_id' : battle_ship, 'timestamp' : now, 'created' : seconds, 'primary_support' : primary_support, 'secondary_support' : secondary_support})
	for k in range(cannons):
		cannon(u_id, battle_ship,lvl)
	for d in range(missiles):
		missile(u_id, battle_ship,lvl)


	
def create_player_items(u_id):
	...
	battle_ship = db.player_items.insert({'name' : name, 'object' : object, 'type' : type, 'x' : x, 'y' : y, 'u_id' : u_id, 'timestamp' : now, 'created' : seconds })
	# By making it a variable, it will still insert, and the variable becomes the id for the object interred.
	...
	for a in range(random.randint(8,15)):
...
def player(u_id):
		...
		db.player_items.update({'_id' : u_id},{"$set": {'race' : race, 'start_date' : start_date, 'strength' : strength, 'experience' : experience }})
